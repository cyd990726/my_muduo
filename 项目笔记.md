...
## 适当使用explicit关键字
对于单参数的构造函数，使用explicit关键字可以防止在赋值的时候隐式类型转换。
...
EventLoop是反应堆的角色，Poller是事件分发器的角色。

### EventLoop
事件循环类，主要包含两大模块：Channel和Poller(epoll的抽象)。特别注意，一个线程里面一个EventLoop，一个EventLoop对应一个Poller，一个Poller对应多个Channel。一个Channel管理着一个socketfd上的事件。

### Channel
Channel可以理解为通道
* fd : 通道对应的文件描述符
* events_: 通道感兴趣事件，例如EPOLLIN, EPOLLOUT等
* revents_: Poller返回以后，在这个fd上发生的具体事件

### EpollPoller
Poller的具体实现类，是对epoll的封装。当epoll_wait监听的sockfd上事件发生时，把发生的事件加入channel中，然后EventLoop就可以根据channel上发生的事件，来调用相应的回调函数。

### 线程模型
mainLoop和各个subLoop之间通过wakeupFd_来通信，当subLoop需要执行回调函数的时候，会通过wakeupFd_来唤醒mainLoop，mainLoop会调用doPendingFunctors()来执行回调函数。


**listen函数** 
在 Linux 网络编程中，listen() 函数的第二个参数（通常记为 backlog）是核心配置项，其核心作用是设定 TCP 监听套接字的未完成连接队列（半连接队列）和已完成连接队列（全连接队列）的总容量上限，本质是限制等待被 accept() 函数处理的连接请求数量
半连接队列：状态为SYN_RECV，客户端发送 SYN 包，服务端回应 SYN+ACK 后，未收到客户端 ACK 前的连接
全连接队列（ACCEPT 队列）：状态为ESTABLISHED，三次握手完成，连接已建立，等待 accept() 函数提取的连接；